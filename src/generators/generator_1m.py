...
class GroupGenerator(object):
    ...
    def __init__(self, data_path, output_path, rating_threshold, num_groups,
        ...
        if verbose:
            ...
            size_raw   = defaultdict(list)  # raw individual member ratings
            size_items = defaultdict(set)   # unique items rated by groups of this size
            size_users = defaultdict(set)   # unique users in groups of this size

            for gid, item, label, _ in group_ratings:
                group = group_id_to_group[gid]
                sz = len(group)
                size_users[sz].update(group)
                ...
                if label == 1:
                    ...
                    size_neg[sz] += 1
                for member in group:
                    raw = rating_mat[member, item]
                    if raw > 0:
                        size_raw[sz].append(int(raw))

            all_sizes = sorted(set(len(g) for g in groups))

            col = [14, 10, 10, 10, 10, 12, 12, 14]
            headers = ['Group Size', '# Groups', '# Users', '# Items',
                       '# Ratings', '% Positive', '% Negative', 'Median Rating']
            ...
            for sz in all_sizes:
                ...
                med_raw   = float(np.median(size_raw[sz])) if size_raw[sz] else float('nan')
            ...
            all_raw     = [r for s in all_sizes for r in size_raw[s]]
            tot_median  = float(np.median(all_raw)) if all_raw else float('nan')
            tot_vals    = ['TOTAL', str(tot_groups), str(tot_users), str(tot_items),
                           str(tot_total),
                           '{:.1f}%'.format(tot_pct_pos),
                           '{:.1f}%'.format(tot_pct_neg),
                           '{:.2f}'.format(tot_median)]
