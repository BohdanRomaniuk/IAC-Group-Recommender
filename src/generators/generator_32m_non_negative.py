...
class GroupGenerator(object):
    ...
    def __init__(self, data_path, output_path, rating_threshold, num_groups,
        ...
        if is_csv:
            # Generate movies.dat filtered to movies that appear in group ratings
            ...
            with open(items_csv_path, 'r', encoding='utf-8') as src, \
                ...
                for line in src:
                    parts = line.rstrip('\n').split(',', 2)
                    if int(parts[0]) in groups_rated_items_set:
                        dst.write('::'.join(parts) + '\n')

            # Generate users.dat filtered to group members only
            ...
            # Filter movies.dat to movies that appear in group ratings
            out_movies_dat = os.path.join(output_path, 'movies.dat')
            with open(os.path.join(data_path, 'movies.dat'), 'r', encoding='iso-8859-1') as src, \
                 open(out_movies_dat, 'w', encoding='iso-8859-1') as dst:
                for line in src:
                    if int(line.split('::')[0]) in groups_rated_items_set:
                        dst.write(line)

            # Filter users.dat to group members only
            out_users_dat = os.path.join(output_path, 'users.dat')
            with open(os.path.join(data_path, 'users.dat'), 'r') as src, \
                 open(out_users_dat, 'w') as dst:
                for line in src:
                    if int(line.split('::')[0]) in members:
                        dst.write(line)
